...
X, y = asd.load_data('FW') # few subset
print(X.shape)
print(y.shape)
...
options = {'c1': 1.0, 'c2': 1.5, 'w':0.7, 'k': 20, 'p':2}

# Call instance of PSO
dimensions = X.shape[1] # dimensions should be the number of features
print("dimensions: %d" % dimensions)
# optimizer.reset()
optimizer = BinaryPSO(n_particles=30, dimensions=dimensions, options=options)

# Perform optimization
print("Calling optimizer")
# 100 iter => none selected
cost, pos = optimizer.optimize(f, print_step=1, iters=300, verbose=1)
...
selected_df.to_csv(cd.OUTPUT_DIR+"/gs_results_pso2_svm_accuracy.csv", sep='\t', encoding='utf-8')
